...
func evaluate_grammar(wg *sync.WaitGroup, g Grammar, data Stack, queue chan <- Stack) Stack {
	defer wg.Done()
	...
	for _, s := range new_stacks {
		wg.Add(1)
		// write in input queue, change recursive to iterative approach
		queue <- s
...
func init_worker_pool(pool_size int, state <-chan bool, queue chan Stack, output chan <- Stack, wg *sync.WaitGroup, gammar Grammar) {
		...
		go func() {
			for ; ; {
				...
				default:
					data, ok := <-queue
					if ok {
						val := evaluate_grammar(wg, gammar, data, queue)
						if val.success {
							output <- val
						}
					}
...
func shutdown_worker(pool_size int, state chan bool) {
	for i := 0; i < pool_size; i++ {
		state <- true
	}
	close(state)
...
func main() {
	...
	state := make(chan bool, cpu)
	// Fixme: possible deadlock if buffered full,
	queue := make(chan Stack, cpu * 64)
	// result channel, single buffer
	result_chan := make(chan Stack, 1)                // single buffer for 1 success value

	// worker counter and context
	var wg = sync.WaitGroup{}

	// initiate fixed worker thread pool
	go init_worker_pool(cpu, state, queue, result_chan, &wg, gram)

	// counter
	wg.Add(1)
	queue <- begin
	//go evaluate_grammar(&wg, gram, begin, queue)

	// Special routine: if all evaluate_grammar routine finish without SUCCESS, detect it and push empty stack
	go func(wg *sync.WaitGroup, out chan <- Stack) {
		wg.Wait()
		out <- NewStack()
	}(&wg, result_chan)
